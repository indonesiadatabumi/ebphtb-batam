        ...
        <div class="page-header">
          <div class="row align-items-center">
            <div class="col-sm mb-2 mb-sm-0">
              <h1 class="page-header-title">Monitoring Kurang Bayar </h1>
            </div>
          ...
          <div class="table-responsive datatable-custom">
            <table id="datatable" class="table table-lg table-bordered table-thead-bordered table-hover table-align-middle card-table"
              ...
              <thead class="thead-light">
                <tr>
                  <th scope="col" rowspan="2" style="vertical-align: middle;" class="text-left pb-1">NOP <br />Kd. Billing</th>
                  ...
                  <th rowspan="2" style="vertical-align: middle;" class="text-center pb-1">Nomor Ketetapan</th>
                ...
                <tr>
                  <td class="pr-0">
                    <span class="h5 text-hover-primary">
                        {{ EbphtbServices::format_nop($d->nop) }}<br />
                        {{ $d->kd_booking }}
                    </span>
                  </td>
                  <td class="text-center pr-2">{{ $d->tgl_rekam_kb }}</td>
                  <td>{{ $d->rekam_year }}</td>
                  <td>{{ $d->rekam_month }}</td>
                  <td class="pl-2">{{ $d->no_ketetapan_kb }}</td>
                  <td class="text-center pl-2">{{ $d->tgl_ketetapan_kb }}</td>
                  <td class="pl-2">{{ $d->nm_pembeli }}</tds>
                  <td class="text-right pr-2">{{ number_format($d->jumlah_kb, 0, ',','.') }}</td>
                  <td>{{ $d->alasan_kb }}</td>
                  <td class="text-center pl-1 pr-1">
                    <span class="legend-indicator {{ (!is_null($d->tgl_validasi_ke1) ? 'bg-success' : 'bg-danger') }}"></span> <br /><span style="font-size: 12px;">{{ $d->tgl_validasi_ke1 }}</span>
                  </td>
                  <td class="text-center pl-1 pr-1"><span class="legend-indicator {{ (!is_null($d->tgl_validasi_ke2) ? 'bg-success' : 'bg-danger') }}"></span><span style="font-size: 12px;">{{ $d->tgl_validasi_ke2 }}</span></td>
                  <td class="text-center pl-1 pr-1"><span class="legend-indicator {{ (!is_null($d->tgl_validasi_ke3) ? 'bg-success' : 'bg-danger') }}"></span><span style="font-size: 12px;">{{ $d->tgl_validasi_ke3 }}</span></td>
                  <td class="pl-2 pr-1 text-center"><a href="{{ url('/kb/cetak/') }}/{{ $d->kd_booking }}" class="btn btn-soft-primary btn-sm btn-pill"><i class="tio-print"></i> Cetak </a></td>
                </tr>
                @endforeach
              </tbody>
            </table>
        ...
        */
        ...
        var datatable = $.HSCore.components.HSDatatables.init($('#datatable'), {
          ...
          initComplete: function(){
            this.api().column(2).search('2025').draw(); // get current year
            this.api().column(3).search('5').draw(); // get current month
          }
        });
